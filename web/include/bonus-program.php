<li nk="" href="https://fonts.googleapis.com/css?family=Pacifico" rel="stylesheet">
<div id="about_wrap">
	<div class="about_body">
		<div class="about_head">
			<h1 class="b-title">Бонусная программа</h1>
		</div>
		<div class="about_asvantages_head">
			<div>
 <img src="/include/static/images/top_bul.png" alt="">Быстрая<br>
				 оплата
			</div>
			<div>
 <img src="/include/static/images/top_bul.png" alt="">Выгодные<br>
				 условия
			</div>
			<div>
 <img src="/include/static/images/top_bul.png" alt="">Подарочные<br>
				 бонусы
			</div>
			<div>
 <img src="/include/static/images/top_bul.png" alt="">Оплата любых<br>
				 товаров
			</div>
		</div>
		<div class="clear">
		</div>
	</div>
	<div class="bonus_top_block">
		<div class="left_top_block">
 <img src="/include/static/images/bonus_left.jpg" alt="">
			<div class="right_top_block">
				<div class="right_text_wrap">
					<h3 class="right_text_wrap__title">Стань участником<br>
					 бонусной программы</h3>
					<p>
						 Бонусная программа&nbsp;позволит получать накопительные бонусы на карту Четыре Лапы. Копите или тратьте бонусные баллы на любые покупки в магазине, на сайте или в мобильном приложении, получайте дополнительные подарочные бонусы и совершайте покупки с выгодой!
					</p>
				</div>
			</div>
		</div>
		<div class="clear">
		</div>
	</div>
	<div class="green_ribbon">
		<div class="ribbon_body">
			<h2 class="b-title b-title--viewed">Как получить карту?</h2>
			<hr class="green_line">
			<p>
				 Для того&nbsp;чтобы получить бонусную карту необходимо сделать 3 простых шага
			</p>
		</div>
	</div>
	<div class="bonus_body">
		<div class="bonus_middle_block">
			<div class="bonus_wrap">
				<div class="bonus_item">
					<h3>Шаг 1</h3>
 <img src="/include/static/images/bon_3.jpg" alt="">
					<p>
						 Сделайте единовременную покупку любых товаров на сумму от 500 рублей <span class="hide_block_wrap">*<span class="hide_block">Для городов Волгоград, Воронеж, Калуга, Владимир, Иваново, Кострома, Липецк, Луховицы, Нижний Новгород, Орел, Рязань, Тверь, Ярославль - покупка на 300 рублей.</span></span>
					</p>
				</div>
				<div class="bonus_item">
					<h3>Шаг 2</h3>
 <img src="/include/static/images/bon_2.jpg" alt="">
					<p>
						 Получите карту на кассе или виртуальную карту на сайте или в мобильном приложении
					</p>
				</div>
				<div class="bonus_item">
					<h3>Шаг 3</h3>
 <img src="/include/static/images/bon_1.jpg" alt="">
					<p>
						 Оплачивайте до 90% следующих покупок бонусами
					</p>
				</div>
				<div class="clear">
				</div>
			</div>
		</div>
		<div class="clear">
		</div>
	</div>
	<div class="green_ribbon">
		<div class="ribbon_body">
			<h2 class="b-title b-title--viewed">Преимущества бонусной программы</h2>
			<hr class="green_line">
		</div>
	</div>
	<div class="option_body bonus_advantages_body">
		<div class="about_middle_block">
			<div class="column_wrap">
				<div class="column">
					<div class="column_top">
 <img src="/include/static/images/ic5_3.png" alt="">
						<div class="column_text_wrap">
							<h3>Быстро!</h3>
							<div class="column_p_wrap">
								<p>
									 Тратить бонусы можно сразу после регистрации<br>
								</p>
							</div>
						</div>
					</div>
				</div>
				<div class="column">
					<div class="column_top">
 <img src="/include/static/images/ic5_1.png" alt="">
						<div class="column_text_wrap">
							<h3>Удобно!</h3>
							<div class="column_p_wrap">
								<p>
									 Оплачивайте до 90% следующих покупок
								</p>
							</div>
						</div>
					</div>
				</div>
				<div class="column">
					<div class="column_top">
 <img src="/include/static/images/ic5_2.png" alt="">
						<div class="column_text_wrap">
							<h3>Выгодно!</h3>
							<div class="column_p_wrap">
								<p>
									 Один начисленный бонус равен одному рублю
								</p>
							</div>
						</div>
					</div>
				</div>
				<div class="clear">
				</div>
			</div>
		</div>
		<div class="clear">
		</div>
	</div>
	<div class="bonus_top_block" id="bonus_bottom">
		<div class="left_top_block">
 <img src="/include/static/images/bonus_banner.jpg" alt="">
			<div class="right_top_block">
				<div class="right_text_wrap">
					<h3 class="right_text_wrap__title">Виртуальная бонусная&nbsp;карта</h3>
					<p>
						 Больше не&nbsp;нужно носить карту с&nbsp;собой! Скачивайте приложение на&nbsp;App Store или Google Play, регистрируйте карту и&nbsp;используйте смартфон для оплаты покупок бонусами в&nbsp;приложении или в&nbsp;магазине Четыре Лапы.
					</p>
 <a class="b-common-item__add-to-cart" href="https://itunes.apple.com/us/app/%D1%87%D0%B5%D1%82%D1%8B%D1%80%D0%B5-%D0%BB%D0%B0%D0%BF%D1%8B-%D0%B7%D0%BE%D0%BE%D0%BC%D0%B0%D0%B3%D0%B0%D0%B7%D0%B8%D0%BD/id1222315361?mt=8" title="" tabindex="0" target="_blank"> <img src="/include/static/images/app-store.png" alt=""> </a> <a class="b-common-item__add-to-cart" href="https://play.google.com/store/apps/details?id=com.appteka.lapy&hl=ru" title="" tabindex="0" target="_blank"> <img src="/include/static/images/android.png" alt=""> </a>
				</div>
			</div>
		</div>
		<div class="clear">
		</div>
	</div>
	<div class="phone_ribbon">
		<div class="phone_ribbon_body partners_phone">
			<p>
				 Бонусы не начисляются на корма для кошек и собак следующих торговых марок: <br>
				<b>WHISKAS</b>, <b>KITEKAT</b>, <b>FELIX</b>, <b>FRISKIES</b>, <b>PERFECT FIT</b>, <b>SHEBA</b>, <b>GOURMET</b>, <b>CEASAR</b>, <b>PEDIGREE</b>, <b>CHAPPI</b>.
			</p>
 <br>
			<div class="links_block">
 <a class="b-common-item__add-to-cart" href="/upload/rules/pravila_bonusnoy_programmy.pdf" target="_blank" title="" tabindex="0"> <span class="b-common-item__wrapper-link"> <span class="b-common-item__price js-price-block">Правила&nbsp;бонусной&nbsp;программы&nbsp;&gt;&nbsp;&nbsp;</span> </span> </a>
			</div>
		</div>
	</div>
	<div class="phone_ribbon">
		<div class="phone_ribbon_body partners_phone">
			<h3 class="phone_ribbon_body__title">Задайте нам вопрос!</h3>
			<p>
				 Позвоните по телефону <b>8-800-770-00-22</b> или закажите обратный звонок.
			</p>
		</div>
	</div>
</div>
 </li>