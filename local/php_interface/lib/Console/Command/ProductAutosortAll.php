...
class ProductAutosortAll extends Command implements LoggerAwareInterface
{
    ...
    {
        $this->setName('productautosort:all')
             ->setDescription('Perform autosorting on ALL PRODUCTS in catalog.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $dbAllProducts = CIBlockElement::GetList(
            ['ID' => 'ASC'],
        ...
        $this->statProductTotal = $dbAllProducts->SelectedRowsCount();

        $this->log()->info(
            sprintf(
                'Всего товаров в каталоге: %d',
                $this->statProductTotal
            )
        );
        ...
        while ($product = $dbAllProducts->Fetch()) {
            ...
            if ($CIBlockElement->SetElementSection($product['ID'], $productToCategories[$product['ID']])) {
