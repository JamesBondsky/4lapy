...
class News20171128132638 extends SprintMigrationBase
{
    ...
    public function up()
    {
        ...
        $aIblock = $helper->Iblock()->getIblock('news', 'publications');
        if (!empty($aIblock)) {
            $ib = new \CIBlock();
            $ib->Update(
                $aIblock['ID'],
                [
                    'LIST_PAGE_URL'    => '#SITE_DIR#/company/news/',
                    'DETAIL_PAGE_URL'  => '#SITE_DIR#/company/news/#ELEMENT_CODE#/',
                    'SECTION_PAGE_URL' => '#SITE_DIR#/company/news/',
                ]
            );
        } else {
