...
class CBitrixLocationSelectorSystemComponentPropLocation extends CBitrixLocationSelectorSearchComponent
{
     ...
     */
    protected static function getEntityListByListOfPrimary($entityClass, $list = [],
                                                           array $parameters = [],
                                                           $fieldCode = 'ID') : \Bitrix\Main\DB\ArrayResult
    {
        $result = array();
    ...
    protected function obtainDataConnectors()
    {
        ...
        if($this->arParams['PROP_LOCATION'] === 'Y'){
            ...
            {
                $res =
                    self::getEntityListByListOfPrimary(self::LOCATION_ENTITY_NAME,
        ...
        if($this->useGroups)
        {
            ...
            if($this->arParams['PROP_LOCATION'] === 'Y'){
                $res =
                    self::getEntityListByListOfPrimary(GroupTable::class,
                ...
                if($this->groupsFromRequest !== false) {
                    $res = self::getEntityListByListOfPrimary(GroupTable::class,
                                                              $this->groupsFromRequest,
                                                              $parameters,
                                                              $linkFld);
                }
    ...
    public static function processGetPathRequest($parameters)
    {
        ...
        $res = self::getEntityListByListOfPrimary(self::LOCATION_ENTITY_NAME, $idList, array('select' => array('ID', 'LEFT_MARGIN', 'RIGHT_MARGIN')));
        while($item = $res->fetch()) {
            $items[] = $item;
        }
