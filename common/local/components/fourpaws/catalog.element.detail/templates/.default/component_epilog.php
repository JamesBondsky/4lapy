...
foreach ($offers as $offer) {
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $offerLink.find('.b-weight-container__price').html('<?= WordHelper::numberFormat($offer->getPrice(), 0) ?> <span class="b-ruble b-ruble--weight">₽</span>');
            ...
            <?php }
                ...
                if($offer->getOldPrice() !== $offer->getPrice()){?>
                    $offerLink.find('.b-weight-container__old-price--big').html('<?=WordHelper::numberFormat($offer->getOldPrice(), 0)?> <span class="b-ruble b-ruble--old-weight-price">₽</span>').css('display', 'inline-block');
                <?php }
            }?>
        });
    </script>
