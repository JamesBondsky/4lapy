...
class FourPawsOrderShopListComponent extends FourPawsShopListComponent
{
    ...
    protected function prepareResult(array $city = [])
    {
        ...
        if ($routeCollection = $router->getRouteCollection()) {
            $storeListUrlRoute = $routeCollection->get('fourpaws_store_ajax_storelist_getall');
        }
        $storeListUrlRoutePath = $storeListUrlRoute ? $storeListUrlRoute->getPath() : '';
        $this->arResult['STORE_LIST_URL'] = $storeListUrlRoutePath;
