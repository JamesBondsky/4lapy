...
class FourPawsFrontOfficeCardRegistrationComponent extends \CBitrixComponent
{
                ...
                if ($validateResult) {
                    ...
                    if ($card) {
                        ...
                        $this->arResult['CARD_DATA']['USER'] = [
                            'CONTACT_ID' => trim($card->contactId),
                            'LAST_NAME' => trim($card->lastName),
                            'FIRST_NAME' => trim($card->firstName),
                            'SECOND_NAME' => trim($card->secondName),
                            'BIRTHDAY' => date('d.m.Y', strtotime(trim($card->birthDate))),
    ...
    {
        $this->arResult['ERROR'][$fieldName] = new Error($errorMsg, $errCode);
    }
