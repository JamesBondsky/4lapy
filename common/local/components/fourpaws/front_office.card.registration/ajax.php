...
$siteId = isset($_REQUEST['ajaxContext']['siteId']) && is_string($_REQUEST['ajaxContext']['siteId']) ? $_REQUEST['ajaxContext']['siteId'] : '';
...
$siteTemplateId = isset($_REQUEST['ajaxContext']['siteTemplateId']) && is_string($_REQUEST['ajaxContext']['siteTemplateId']) ? trim($_REQUEST['ajaxContext']['siteTemplateId']) : '';
...
try {
    ...
    if ($ajaxContext && is_array($ajaxContext)) {
        $template = $signer->unsign($ajaxContext['template'], 'front_office.card.registration');
        $paramString = $signer->unsign($ajaxContext['parameters'], 'front_office.card.registration');
        $correct = true;
    }
