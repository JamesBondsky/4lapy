...
;(function (window) {
        ...
        act:  function (act, id) {
            if (!id || !this.mess['m' + id]) {
                ...
                while (ii++ < 50 && (selection.search(/\002([^\002\003]*)\003/gi) >= 0 || selection.search(/\001([^\001\003]*)\003/gi) >= 0)) {
                    selection = selection.replace(/\002([^\002\003]*)\003/gi, "[CODE]$1[/CODE]")
                ...
                var
                    ...
                    href            = link.getAttribute("bx-href")
        ...
        d.prototype = {
            ...
            validate:       function () {
                ...
                if (MessageLength <= 0) {
                    ...
                    errors += BX.message('max_len')
                        .replace(/#MAX_LENGTH#/gi, MessageMax)
                        .replace(/#LENGTH#/gi, MessageLength);
                }
                ...
                if (this.editor.GetViewMode() === 'wysiwyg') // BB Codes
                {
                    ...
                    if (author) {
                        if (author.id > 0) {
                            ...
                            }) + '" class="bxhtmled-metion">' + author.name.replace(/</gi, '&lt;')
                                         .replace(/>/gi, '&gt;') + '</span>';
                        } else {
    ...
    window.fReplaceOrInsertNode = function (sourceNode, targetNode, parentTargetNode, beforeTargetNode) {
        ...
        if (!BX.type.isDomNode(parentTargetNode)) {
            return false;
        }
        
        if (!BX.type.isDomNode(sourceNode) && !BX.type.isArray(sourceNode) && sourceNode.length > 0) {
            if (!(sourceNode = window.fTextToNode(sourceNode))) {
                return false;
            }
        }
