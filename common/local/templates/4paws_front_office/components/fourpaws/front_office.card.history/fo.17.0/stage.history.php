...
if (!empty($arResult['CURRENT_CARD'])) {
    echo '<div class="active-balance text-h3">Активный баланс: '
         . htmlspecialcharsbx($arResult['CURRENT_CARD']['BALANCE']) . '&nbsp;баллов</div>';
}
...
if (!empty($arResult['CHEQUES'])) {
    ...
    <div class="print-version-request-form">
        <form method="post" action="" target="_blank">
            ...
            <input type="hidden"
                   name="cardNumberForHistory"
                   value="<?= $arResult['PRINT_FIELDS']['cardNumberForHistory']['VALUE'] ?>">
        ...
        foreach ($arResult['CHEQUES'] as $cheque) {
            ...
            <tr class="order-list__head <?= $rowClass ?>">
                <td class="order-list__i">
                        <span class="order-list__dropdown uppercase"
                              data-id="<?= htmlspecialcharsbx($cheque['CHEQUE_ID']) ?>">
