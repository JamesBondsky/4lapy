...
if ($arResult['USE_AJAX'] === 'Y' && $arResult['IS_AJAX_REQUEST'] !== 'Y') {
    ...
    $arResult['JS']['signedTemplate'] = $signer->sign($this->GetName(), 'front_office.fest_reg');
    $arResult['JS']['signedParams'] = $signer->sign(
        base64_encode(serialize($arResult['ORIGINAL_PARAMETERS'])),
        'front_office.fest_reg'
    );
}
...
if ($arResult['WAS_POSTED'] === 'Y') {
    $arResult['CURRENT_STAGE'] = 'user_reg';
...
foreach ($printFields as $fieldName) {
    ...
    if (in_array($fieldName, ['firstName', 'phone'])) {
        $arResult['PRINT_FIELDS'][$fieldName]['REQUIRED'] = true;
    }
}
