...
class Kcaptcha {
	...
	function Kcaptcha($w, $h, $bg_color, $color){
		...
		if($bg_color) {
			$tmp_color = hexdec($bg_color);
			$b = intval($tmp_color%256);
			$g = intval($tmp_color/256%256);
			$r = intval($tmp_color/(256*256));
			$background_color = array($r, $g, $b);
		}
		if($color) {
			$tmp_color = hexdec($color);
			$b = intval($tmp_color%256);
			$g = intval($tmp_color/256%256);
			$r = intval($tmp_color/(256*256));
			$foreground_color = array($r, $g, $b);
		}
		$fonts=array();
