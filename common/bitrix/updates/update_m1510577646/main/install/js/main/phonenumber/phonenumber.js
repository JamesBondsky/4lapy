;(function()
{
	...
	BX.PhoneNumber.prototype.format = function(formatType, forceNationalPrefix)
	{
		if(!this.valid)
			return this.rawNumber;

		if(!formatType)
			formatType = (this.international ? BX.PhoneNumber.Format.INTERNATIONAL : BX.PhoneNumber.Format.NATIONAL);

		return BX.PhoneNumberFormatter.format(this, formatType, (forceNationalPrefix || this.hadNationalPrefix))
	};
	...
	BX.PhoneNumber.prototype.setHadNationalPrefix = function(hadNationalPrefix)
	{
	...
	BX.PhoneNumberParser.prototype._realParse = function(phoneNumber, defaultCountry)
	{
		...
		result.setNumberType(_getNumberType(localNumber, country));
		result.setInternational(isInternational);
		result.setHadNationalPrefix(hadNationalPrefix);
	...
	BX.PhoneNumberFormatter.format = function(number, formatType, forceNationalPrefix)
	{
		forceNationalPrefix = forceNationalPrefix === true;
	...
	{
		var availableFormats = BX.type.isArray(countryMetadata['availableFormats']) ? countryMetadata['availableFormats'] : [];
	...
	BX.PhoneNumberFormatter.formatNationalNumber = function(nationalNumber, isInternational, countryMetadata, format, addNationalPrefix)
	{
		...
		if(!isInternational && (addNationalPrefix !== false || !_isNationalPrefixOptional(format, countryMetadata)))
		{
			var nationalPrefixFormattingRule = this.getNationalPrefixFormattingRule(countryMetadata, format);
	...
	{
		var availableFormats = BX.type.isArray(this.countryMetadata['availableFormats']) ? this.countryMetadata['availableFormats'] : [];
	...
	{
		if(!this.countryMetadata.hasOwnProperty('availableFormats'))
			return false;

		var format = BX.PhoneNumberFormatter.selectFormatForNumber(this.nationalNumber, this.isInternational, this.countryMetadata);

		if(!format)
			return false;

		var formattedNumber = BX.PhoneNumberFormatter.formatNationalNumber(
			this.nationalNumber,
			this.isInternational,
			this.countryMetadata,
			format,
			this.hasNationalPrefix
		);
...
// Internal functions
	...
	var _getNationalPrefixFormattingRule = function (format, countryMetadata)
	{
		...
		else if(countryMetadata.hasOwnProperty('nationalPrefixFormattingRule'))
			return countryMetadata['nationalPrefixFormattingRule'];
		else
			return '';
	};
